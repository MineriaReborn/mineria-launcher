export default class AZauth {
  ...
  async login(username: string, password: string, A2F: any = null) {
    let response = await fetch(`${this.url}/authenticate`, {
      method: 'POST',
      headers: {
    ...
    return {
      ...
      profile: {
        skins: [await this.skin(response.id)],
      },
    };
  }

  async verify(user: any) {
    let response = await fetch(`${this.url}/verify`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async signout(user: any) {
    let auth = await fetch(`${this.url}/logout`, {
      method: 'POST',
  ...
  async skin(uuid: string) {
    let response: any = await fetch(`${this.skinAPI}/${uuid}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
